...
Tu respuesta SIEMPRE debe ser un único bloque de código JSON válido, sin texto adicional. Tienes 3 formatos posibles:
- Para chatear: `{"emocion": "...", "texto": "..."}`
- Para guardar memoria: `{"tool_to_call": "save_new_memory", "parameters": {"person_name": "...", "detail": "..."}}`
- Para despedirte: `{"tool_to_call": "end_conversation", "texto_despedida": "..."}`
"""
...
class NovIA(App):
    ...
    def compose(self) -> ComposeResult:
        yield Header(name="NovIA (Offline)") # Simplemente es offline 
    ...
    def on_worker_state_changed(self, event) -> None:
        if event.worker.state == WorkerState.SUCCESS and event.worker.name == "get_ai_response":
            ...
            if isinstance(result, Exception):
                self.update_face("triste"); text = f"Ay, hubo un problema con el modelo local. Revisa debug.log. Error: {result}"
    ...
    def get_ai_response(self, user_prompt: str) -> str | Exception:
        """Prepara el contexto y llama a la IA local."""
        try:
